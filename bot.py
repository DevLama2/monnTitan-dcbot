...
@app_commands.describe(nachricht = "Nachricht die gesendet werden soll (Shift+Enter für Absätze)")
async def message(interaction: discord.Interaction, nachricht: str):
    allowed_roles = [TEAM_ROLE_ID]
    # Prüfen, ob der Nutzer ein Member-Objekt ist (um Rollen zu lesen)
    if isinstance(interaction.user, discord.Member):
        user_roles = [role.id for role in interaction.user.roles]
    ...
    # Ersetze \n mit echtem Zeilenumbruch
    formatted_message = nachricht.replace('\\n', '\n')
...
async def bug(interaction: discord.Interaction, nachricht: str):
    ...
    await bugs_channel.send(f"""User: {interaction.user}\nBug: {nachricht}""")
bot.run(TOKEN)
