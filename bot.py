...
# Admin IDs
ADMIN_IDS = [803181060783472661, 793459877275893771]

# Server IDs
LAMA_GUILD_ID = 834345976420106261  # Lama Server für alle Befehle
...
# Channel IDs für Logging
LAMA_LOG_CHANNEL = 1369996338715951174  # Neuer Log Channel auf Lama Server
MESSAGE_LOG_CHANNEL = 1322550417988522014  # Bisheriger Log Channel
TRACKED_CHANNELS = [
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Registriere Views für persistente Buttons
        self.add_view(TicketView())
        self.add_view(CloseView())
        
        try:
            # Sync commands für alle Server separat
            print("\nSynchronisiere Commands...")
            
            # Sync commands für den Lama Server
            synced_lama = await self.tree.sync(guild=discord.Object(id=LAMA_GUILD_ID))
            print("\n🔄 Synchronisierte Commands für Lama Server:")
            for cmd in synced_lama:
                print(f"  ✓ /{cmd.name}")
            print(f"\n✅ {len(synced_lama)} Befehle für Lama Server synchronisiert\n")
            
            # Sync commands für den Hauptserver
            synced_main = await self.tree.sync(guild=discord.Object(id=MAIN_GUILD_ID))
            print("🔄 Synchronisierte Commands für Hauptserver:")
            for cmd in synced_main:
                print(f"  ✓ /{cmd.name}")
            print(f"\n✅ {len(synced_main)} Befehle für Hauptserver synchronisiert\n")
            
            # Sync commands für den Team Server
            synced_team = await self.tree.sync(guild=discord.Object(id=TEAM_GUILD_ID))
            ...
            print(f"❌ Fehler beim Synchronisieren der Commands: {str(e)}")
...
# Neue Modal-Klasse für das Bearbeiten von Nachrichten
class EditModal(discord.ui.Modal, title="Nachricht bearbeiten"):
    message_content = discord.ui.TextInput(
        label="Neue Nachricht",
        style=discord.TextStyle.paragraph,
        placeholder="Gib hier die neue Version der Nachricht ein...",
        required=True,
        max_length=2000
    )
    ...
    def __init__(self, message: discord.Message):
        ...
        self.message_content.default = message.content

    async def on_submit(self, interaction: discord.Interaction):
        try:
            await self.message.edit(content=self.message_content.value)
            await interaction.response.send_message("✅ Die Nachricht wurde erfolgreich bearbeitet!", ephemeral=True)
        except discord.Forbidden:
            await interaction.response.send_message("❌ Ich habe keine Berechtigung, diese Nachricht zu bearbeiten!", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"❌ Fehler beim Bearbeiten der Nachricht: {str(e)}", ephemeral=True)

@bot.tree.command(
    name="edit",
    description="Öffnet ein Formular zum Bearbeiten einer Bot-Nachricht",
    guilds=[discord.Object(id=MAIN_GUILD_ID), discord.Object(id=TEAM_GUILD_ID)]
)
@app_commands.describe(
    message_id="Die ID der zu bearbeitenden Nachricht"
)
async def edit(interaction: discord.Interaction, message_id: str):
    ...
    try:
        ...
        # Prüfe ob es eine Bot-Nachricht ist
        if message.author != bot.user:
            await interaction.response.send_message("❌ Du kannst nur Nachrichten des Bots bearbeiten!", ephemeral=True)
            return
            
        # Öffne das Modal-Formular
        await interaction.response.send_modal(EditModal(message))
        
    except discord.NotFound:
        await interaction.response.send_message("❌ Die angegebene Nachricht wurde nicht gefunden!", ephemeral=True)
    except ValueError:
        await interaction.response.send_message("❌ Ungültige Nachrichten-ID! Bitte gib eine gültige ID an.", ephemeral=True)
    except Exception as e:
    ...
    async def create_ticket(self, interaction: discord.Interaction, ticket_type: str):
        try:
            ...
            if ticket_type == "support":
                ...
                description = f"Hallo {interaction.user.mention}!\nBitte beschreibe dein Anliegen. Ein Teammitglied wird sich so schnell wie möglich um dich kümmern.\nWenn du deine Frage beantwortet und aufgenommen wurde, dann klicke bitte auf den Button unten, um das Ticket zu schließen!"
            else:
                title = "Bewerbungsticket"
                description = f"Hallo {interaction.user.mention}!\nBitte stelle dich vor und beschreibe, warum du dich für das Team bewerben möchtest. Ein Teammitglied wird sich so schnell wie möglich bei dir melden. Sobald deine Bewerbung entweder angenommen oder abgelehnt wurde, klicke bitte auf den Button unten, um das Ticket zu schließen!"
...
@bot.tree.command(name="killswitch", description="⚠️ WARNUNG: Löscht den Bot permanent!", 
                  guilds=[discord.Object(id=MAIN_GUILD_ID), discord.Object(id=TEAM_GUILD_ID)])
async def killswitch(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message("☠️ Bot wird permanent gelöscht...", ephemeral=True)
        ...
        await interaction.followup.send(f"❌ Fehler beim Ausführen des Killswitch: {str(e)}", ephemeral=True)

# Logging Funktion für den Lama Server
async def log_to_lama_channel(bot, message: str, embed: discord.Embed = None):
    log_channel = bot.get_channel(LAMA_LOG_CHANNEL)
    if log_channel:
        try:
            if embed:
                await log_channel.send(embed=embed)
...
# Formular für Rollenberechtigungen
class RolePermissionsModal(discord.ui.Modal, title="Rollenberechtigungen"):
    def __init__(self, role_or_user, is_role=True):
        super().__init__()
        self.role_or_user = role_or_user
        self.is_role = is_role

    admin = discord.ui.TextInput(
        label="Admin-Rechte (ja/nein)",
        required=True,
        placeholder="ja oder nein eingeben"
    )

    manage_messages = discord.ui.TextInput(
        label="Nachrichten verwalten (ja/nein)",
        required=True,
        placeholder="ja oder nein eingeben"
    )

    manage_roles = discord.ui.TextInput(
        label="Rollen verwalten (ja/nein)",
        required=True,
        placeholder="ja oder nein eingeben"
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            # Konvertiere Eingaben in Berechtigungen
            admin_perm = self.admin.value.lower() == "ja"
            msg_perm = self.manage_messages.value.lower() == "ja"
            role_perm = self.manage_roles.value.lower() == "ja"

            # Erstelle die Berechtigungen
            perms = discord.Permissions()
            perms.administrator = admin_perm
            perms.manage_messages = msg_perm
            perms.manage_roles = role_perm

            if self.is_role:
                await self.role_or_user.edit(permissions=perms)
                entity_name = self.role_or_user.name
                entity_type = "Rolle"
            else:
                # Für Benutzer müssen wir die Rollen entsprechend anpassen
                # Dies ist ein vereinfachtes Beispiel
                for role in interaction.guild.roles:
                    if role.permissions == perms:
                        await self.role_or_user.add_roles(role)
                        break
                entity_name = self.role_or_user.name
                entity_type = "Benutzer"

            # Erstelle Embed für das Logging
            embed = discord.Embed(
                title="Berechtigungen aktualisiert",
                color=discord.Color.green(),
                timestamp=datetime.now()
            )
            embed.add_field(name=f"{entity_type}", value=entity_name, inline=False)
            embed.add_field(name="Geändert von", value=interaction.user.mention, inline=False)
            embed.add_field(name="Admin", value="✅" if admin_perm else "❌", inline=True)
            embed.add_field(name="Nachrichten verwalten", value="✅" if msg_perm else "❌", inline=True)
            embed.add_field(name="Rollen verwalten", value="✅" if role_perm else "❌", inline=True)

            # Sende Log
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ Berechtigungen für {entity_type} {entity_name} wurden aktualisiert!", 
                ephemeral=True
            )

        except Exception as e:
            await interaction.response.send_message(
                f"❌ Fehler beim Aktualisieren der Berechtigungen: {str(e)}", 
                ephemeral=True
            )

@bot.tree.command(
    name="give",
    description="Öffnet ein Formular zum Einstellen von Berechtigungen",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
@app_commands.describe(
    target="@Rolle oder @User",
    server_id="Die Server-ID"
)
async def give(interaction: discord.Interaction, target: str, server_id: str):
    try:
        ...
        # Versuche zuerst als Rolle zu finden
        role = discord.utils.get(guild.roles, mention=target) or discord.utils.get(guild.roles, name=target)
        if role:
            await interaction.response.send_modal(RolePermissionsModal(role, is_role=True))
            return

        # Versuche als Benutzer zu finden
        user = discord.utils.get(guild.members, mention=target) or discord.utils.get(guild.members, name=target)
        if user:
            await interaction.response.send_modal(RolePermissionsModal(user, is_role=False))
            return

        await interaction.response.send_message("❌ Rolle oder Benutzer nicht gefunden!", ephemeral=True)
...
@bot.tree.command(
    name="ban",
    description="Bannt einen Benutzer oder eine Rolle",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
@app_commands.describe(
    target="@Rolle oder @User",
...
async def ban(interaction: discord.Interaction, target: str, server_id: str):
    try:
        guild = bot.get_guild(int(server_id))
        if not guild:
            await interaction.response.send_message("❌ Server nicht gefunden!", ephemeral=True)
        ...
        embed.add_field(name="Ausgeführt von", value=interaction.user.mention, inline=False)
        embed.add_field(name="Server", value=guild.name, inline=False)

        # Versuche als Rolle zu bannen
        role = discord.utils.get(guild.roles, mention=target) or discord.utils.get(guild.roles, name=target)
        if role:
            # Banne alle Mitglieder mit dieser Rolle
            banned_users = 0
            for member in role.members:
                try:
                    await member.ban(reason=f"Rollen-Ban durch {interaction.user}")
                    banned_users += 1
                except:
                    continue
            
            embed.add_field(name="Gebannte Rolle", value=role.name, inline=True)
            embed.add_field(name="Anzahl gebannter User", value=str(banned_users), inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ {banned_users} Benutzer mit der Rolle {role.name} wurden gebannt!", 
                ephemeral=True
            )
            return

        # Versuche als Benutzer zu bannen
        user = discord.utils.get(guild.members, mention=target) or discord.utils.get(guild.members, name=target)
        if user:
            await user.ban(reason=f"Ban durch {interaction.user}")
            
            embed.add_field(name="Gebannter User", value=f"{user.name} ({user.id})", inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ Benutzer {user.name} wurde gebannt!", 
                ephemeral=True
            )
            return
...
@bot.tree.command(
    name="kick",
    description="Kickt einen Benutzer oder eine Rolle",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
@app_commands.describe(
    target="@Rolle oder @User",
...
async def kick(interaction: discord.Interaction, target: str, server_id: str):
    try:
        guild = bot.get_guild(int(server_id))
        if not guild:
            await interaction.response.send_message("❌ Server nicht gefunden!", ephemeral=True)
        ...
        # Versuche als Rolle zu kicken
        role = discord.utils.get(guild.roles, mention=target) or discord.utils.get(guild.roles, name=target)
        if role:
            # Kicke alle Mitglieder mit dieser Rolle
            kicked_users = 0
            for member in role.members:
                try:
                    await member.kick(reason=f"Rollen-Kick durch {interaction.user}")
                    kicked_users += 1
                except:
                    continue
            
            embed.add_field(name="Gekickte Rolle", value=role.name, inline=True)
            embed.add_field(name="Anzahl gekickter User", value=str(kicked_users), inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ {kicked_users} Benutzer mit der Rolle {role.name} wurden gekickt!", 
                ephemeral=True
            )
            return

        # Versuche als Benutzer zu kicken
        user = discord.utils.get(guild.members, mention=target) or discord.utils.get(guild.members, name=target)
        if user:
            await user.kick(reason=f"Kick durch {interaction.user}")
            
            embed.add_field(name="Gekickter User", value=f"{user.name} ({user.id})", inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ Benutzer {user.name} wurde gekickt!", 
                ephemeral=True
            )
            return
...
@bot.tree.command(
    name="nickname",
    description="Ändert den Nickname eines Benutzers oder aller Benutzer mit einer Rolle",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
@app_commands.describe(
    target="@Rolle oder @User",
    server_id="Die Server-ID",
    new_name="Der neue Nickname"
)
async def nickname(interaction: discord.Interaction, target: str, server_id: str, new_name: str):
    try:
        guild = bot.get_guild(int(server_id))
        if not guild:
            await interaction.response.send_message("❌ Server nicht gefunden!", ephemeral=True)
        ...
        embed.add_field(name="Geändert von", value=interaction.user.mention, inline=False)
        embed.add_field(name="Server", value=guild.name, inline=False)
        embed.add_field(name="Neuer Name", value=new_name, inline=False)

        # Versuche als Rolle zu ändern
        role = discord.utils.get(guild.roles, mention=target) or discord.utils.get(guild.roles, name=target)
        if role:
            # Ändere Nickname aller Mitglieder mit dieser Rolle
            changed_users = 0
            for member in role.members:
                try:
                    await member.edit(nick=new_name, reason=f"Massen-Nickname-Änderung durch {interaction.user}")
                    changed_users += 1
                except:
                    continue
            
            embed.add_field(name="Geänderte Rolle", value=role.name, inline=True)
            embed.add_field(name="Anzahl geänderter User", value=str(changed_users), inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ Nickname von {changed_users} Benutzern mit der Rolle {role.name} wurde geändert!", 
                ephemeral=True
            )
            return

        # Versuche als Benutzer zu ändern
        user = discord.utils.get(guild.members, mention=target) or discord.utils.get(guild.members, name=target)
        if user:
            await user.edit(nick=new_name, reason=f"Nickname-Änderung durch {interaction.user}")
            
            embed.add_field(name="Geänderter User", value=f"{user.name} ({user.id})", inline=True)
            await log_to_lama_channel(interaction.client, "", embed=embed)
            
            await interaction.response.send_message(
                f"✅ Nickname von {user.name} wurde zu {new_name} geändert!", 
                ephemeral=True
            )
            return
...
# Modifizierter killswitch Command - nur für Lama Server
@bot.tree.command(
    name="killswitch",
    description="⚠️ WARNUNG: Löscht den Bot permanent!",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
async def killswitch(interaction: discord.Interaction):
    if interaction.guild_id != LAMA_GUILD_ID:
        await interaction.response.send_message("⛔ Dieser Befehl ist nur auf dem Lama-Server verfügbar!", ephemeral=True)
        return
    ...
    try:
        ...
        # Logge die Aktion
        embed = discord.Embed(
            title="KILLSWITCH AKTIVIERT",
            color=discord.Color.red(),
            timestamp=datetime.now()
        )
        embed.add_field(name="Ausgeführt von", value=f"{interaction.user.name} ({interaction.user.id})", inline=False)
        await log_to_lama_channel(interaction.client, "", embed=embed)
...
async def log_to_lama(bot, title: str, description: str, color: discord.Color = discord.Color.blue()):
    """Sendet eine Log-Nachricht zum Lama Discord Log Channel"""
    log_channel = bot.get_channel(LAMA_LOG_CHANNEL)
    if log_channel:
        embed = discord.Embed(
            title=title,
...
# Permissions für Rollen und Benutzer
class Permissions:
    VIEW_CHANNEL = 1 << 10
    SEND_MESSAGES = 1 << 11
    ...
    def get_all_permissions():
        return {
            "Kanal anzeigen": Permissions.VIEW_CHANNEL,
            "Nachrichten senden": Permissions.SEND_MESSAGES,
            "Nachrichtenverlauf anzeigen": Permissions.READ_MESSAGE_HISTORY,
            "Nachrichten verwalten": Permissions.MANAGE_MESSAGES,
            "Kanäle verwalten": Permissions.MANAGE_CHANNELS,
            "@everyone/@here erwähnen": Permissions.MENTION_EVERYONE,
            "Dateien anhängen": Permissions.ATTACH_FILES,
            "Reaktionen hinzufügen": Permissions.ADD_REACTIONS,
            "Links einbetten": Permissions.EMBED_LINKS
        }

def calculate_permissions(selected_permissions):
    """Berechnet die Berechtigungsbits basierend auf ausgewählten Berechtigungen"""
    permission_bits = 0
...
class RoleSelectView(discord.ui.View):
    def __init__(self, guild):
        ...
        # Füge Select-Menü für Rollen hinzu
        select_roles = discord.ui.RoleSelect(
            placeholder="Wähle Rollen aus...",
            min_values=0,
            max_values=25
        )
        
        self.add_item(select_roles)
...
class UserSelectView(discord.ui.View):
    def __init__(self, guild):
        ...
        # Füge Select-Menü für Benutzer hinzu
        select_users = discord.ui.UserSelect(
            placeholder="Wähle Benutzer aus...",
            min_values=0,
            max_values=25
        )
        
        self.add_item(select_users)
...
class PermissionSelectView(discord.ui.View):
    def __init__(self):
        ...
        # Füge Select-Menü für Berechtigungen hinzu
        select_permissions = discord.ui.Select(
            placeholder="Wähle Berechtigungen aus...",
            min_values=0,
            max_values=len(Permissions.get_all_permissions()),
            options=[
                discord.SelectOption(
                    label=name,
                    value=str(value)
                ) for name, value in Permissions.get_all_permissions().items()
            ]
        )
        
        self.add_item(select_permissions)
...
class DiscordStylePermissions(discord.ui.View):
    def __init__(self, target_id: int, is_role: bool = True):
        super().__init__(timeout=None)
        ...
        # Add category selects
        for category, perms in self.permissions.items():
            select = discord.ui.Select(
                custom_id=f"perm_{category.lower()}",
                placeholder=f"{category} Permissions",
                min_values=0,
                ...
                options=[
                    discord.SelectOption(
                        ...
                        value=name,
                        description=f"Toggle {name.replace('_', ' ')} permission",
                        emoji="✅" if value else "❌"
                    ) for name, value in perms.items()
                ]
            )
            select.callback = self.permission_callback
    ...
    async def permission_callback(self, interaction: discord.Interaction):
        try:
            ...
            # Update permissions
            for perm in self.permissions[category.title()]:
                self.permissions[category.title()][perm] = perm in selected

            # Update select menu options
            for child in self.children:
                if isinstance(child, discord.ui.Select) and child.custom_id == f"perm_{category}":
                    child.options = [
                        discord.SelectOption(
                            label=name.replace("_", " ").title(),
                            value=name,
                        ...
                        ) for name in self.permissions[category.title()]
                    ]
            ...
            await log_to_lama(
                interaction.client,
                "Permissions Updated",
                f"{'Role' if self.is_role else 'User'} <@{'&' if self.is_role else ''}{self.target_id}> permissions updated in {category}",
                discord.Color.blue()
            )
    ...
    async def save_permissions(self, interaction: discord.Interaction):
        try:
            # Convert permissions to discord.Permissions object
            perms = discord.Permissions()
            for category in self.permissions.values():
            ...
            # Apply permissions
            if self.is_role:
                role = interaction.guild.get_role(self.target_id)
                if role:
                    await role.edit(permissions=perms)
            else:
                member = interaction.guild.get_member(self.target_id)
                if member:
                    # Find or create a role with these permissions
                    role_name = f"Custom Permissions {self.target_id}"
                    role = discord.utils.get(interaction.guild.roles, name=role_name)
                    if not role:
                        role = await interaction.guild.create_role(name=role_name, permissions=perms)
                    await member.add_roles(role)

            # Log the final changes
            embed = discord.Embed(
                title="Permissions Saved",
                description=f"Final permissions for {'role' if self.is_role else 'user'} <@{'&' if self.is_role else ''}{self.target_id}>",
                color=discord.Color.green(),
                timestamp=datetime.now()
            )
            ...
            for category, perms in self.permissions.items():
                enabled_perms = [name.replace("_", " ").title() for name, value in perms.items() if value]
                if enabled_perms:
                    embed.add_field(
            ...
            await log_to_lama(interaction.client, "", embed=embed)
            await interaction.response.send_message("✅ Permissions saved successfully!", ephemeral=True)

        except Exception as e:
            await interaction.response.send_message(f"Error saving permissions: {str(e)}", ephemeral=True)

@bot.tree.command(
    name="permissions",
    description="Open Discord-style permission settings for a role or user",
    guild=discord.Object(id=LAMA_GUILD_ID)
)
@app_commands.describe(
    target="@Role or @User to configure permissions for"
)
async def permissions(interaction: discord.Interaction, target: str):
    try:
        # Try to find role first
        role = discord.utils.get(interaction.guild.roles, mention=target) or discord.utils.get(interaction.guild.roles, name=target)
        if role:
            view = DiscordStylePermissions(role.id, is_role=True)
            embed = discord.Embed(
                title="Role Permissions",
                description=f"Configure permissions for role {role.mention}",
                color=discord.Color.blue()
            )
        ...
        # Try to find user
        user = discord.utils.get(interaction.guild.members, mention=target) or discord.utils.get(interaction.guild.members, name=target)
        if user:
            view = DiscordStylePermissions(user.id, is_role=False)
            embed = discord.Embed(
                title="User Permissions",
                description=f"Configure permissions for user {user.mention}",
                color=discord.Color.blue()
            )
            await interaction.response.send_message(embed=embed, view=view)
            return

        await interaction.response.send_message("❌ Role or user not found!", ephemeral=True)
